...
if [[ "${WARDEN_PARAMS[0]}" == "up" ]] || [[ "${WARDEN_PARAMS[0]}" == "start" ]]; then
    if [[ $($WARDEN_BIN env ps --services | grep php-fpm) ]] && [ $($WARDEN_BIN env exec php-fpm "id" "-u" "www-data") != ${HOST_UID} ]; then
    ...
    if [[ $($WARDEN_BIN env ps --services | grep php-debug) ]] && [ $($WARDEN_BIN env exec php-debug "id" "-u" "www-data") != ${HOST_UID} ]; then
    ...
    if [[ $($WARDEN_BIN env ps --services | grep php-spx) ]] && [ $($WARDEN_BIN env exec php-spx "id" "-u" "www-data") != ${HOST_UID} ]; then
